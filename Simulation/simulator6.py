# -*- coding: utf-8 -*-
"""
Created on Wed Apr 20 10:27:13 2022
...
# ver5: the top wall is pushed down
...
class simulator:
    ...
    def evaluate(indices):
        ...
        #Nstiff=N/2 #number of stiff particles
        #Nsoft=N-Nstiff #number of soft particles
        ...
        #ep=-4*A/Ly #compresssion strain [Lx x]->[Lx x]*(1+ep)
        ...
        while np.amax([np.amax(ax_old),np.amax(ay_old),abs(awy_old)])>alphacut or np.amax([np.amax(ax_old),np.amax(ay_old),awy_old])==0:
            ...
            Fx[ii] = Fx[ii] - deltaF
            #print(deltaF)
            #Fx[ii]=Fx[ii]-2*Kw*np.multiply(np.divide(K[ii],(Kw+K[ii])),dw) #K[ii]*dw
            ...
            Fy[ii] = Fy[ii] - deltaF
            #print(F)
            #Fy[ii]=Fy[ii]-2*Kw*np.multiply(np.divide(K[ii],(Kw+K[ii])),dw) #K[ii]*dw
        ...
        #Fwl[ii]=-2*Kw*np.multiply(np.divide(K[ii],(Kw+K[ii])),dw)
    ...
    def showPackingWithContacts(indices, ratio, Ftest, save=0):

        #### first config
        indices1=np.array(indices[0:23])
        ...
        #Fa=-0.51   #force applied to the top wall
